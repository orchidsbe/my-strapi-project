...
export default {
    async afterCreate(event) {
        ...
        strapi.log.info('Support-request afterCreate triggered for ID:', result.id);
        ...
        `;
                ...
                await strapi.db
                    .query('api::support-request.support-request')
                    .update({
                        where: { id: result.id },
                        data: { emailSent: true },
                    });

                strapi.log.info(`Email отправлен (Support-request)`);
            } catch (err) {
                strapi.log.error('Ошибка отправки email (Support-request):', err);
            }
        }
    },
};
