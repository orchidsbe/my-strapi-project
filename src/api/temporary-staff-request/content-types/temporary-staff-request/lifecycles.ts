...
export default {
    async afterCreate(event) {
        ...
        strapi.log.info('Temporary-staff-request afterCreate triggered for ID:', result.id);
        ...
        `;
                ...
                await strapi.db
                    .query('api::temporary-staff-request.temporary-staff-request')
                    .update({
                        where: { id: result.id },
                        data: { emailSent: true },
                    });

                strapi.log.info(`Email отправлен (Temporary-staff-request)`);
            } catch (err) {
                strapi.log.error('Ошибка отправки email (Temporary-staff-request):', err);
            }
        }
    },
};
