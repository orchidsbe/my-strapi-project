...
export default {
    async afterCreate(event) {
        ...
        strapi.log.info('Outstaffing Request afterCreate triggered for ID:', result.id);
        ...
        if (!result.emailSent) {
          ...
          <table style="border-collapse: collapse; width: 100%;">
            ...
            ${result.userFunctions ? `<tr><td style="border:1px solid #ccc;padding:4px;"><strong>Функции пользователя</strong></td><td style="border:1px solid #ccc;padding:4px;">${result.userFunctions}</td></tr>` : ''}
        ...
        `;
                ...
                await strapi.db
                    .query('api::outstaffing-request.outstaffing-request')
                    .update({
                        where: { id: result.id },
                        data: { emailSent: true },
                    });

                strapi.log.info(`Email отправлен (Outstaffing Request)`);
            } catch (err) {
                strapi.log.error('Ошибка отправки email (Outstaffing Request):', err);
            }
        }
    },
};
