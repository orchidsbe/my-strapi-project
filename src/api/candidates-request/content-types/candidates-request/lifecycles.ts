...
export default {
    async afterCreate(event) {
        ...
        strapi.log.info('Candidates-request afterCreate triggered for ID:', result.id);
        ...
        if (!result.emailSent) {
          ...
          <table style="border-collapse: collapse; width: 100%;">
            ...
            ${result.desiredPosition ? `<tr><td style="border:1px solid #ccc;padding:4px;"><strong>Желаемая позиция</strong></td><td style="border:1px solid #ccc;padding:4px;">${result.desiredPosition}</td></tr>` : ''}
        ...
        `;
                ...
                await strapi.db
                    .query('api::candidates-request.candidates-request')
                    .update({
                        where: { id: result.id },
                        data: { emailSent: true },
                    });

                strapi.log.info(`Email отправлен (candidates-request)`);
            } catch (err) {
                strapi.log.error('Ошибка отправки email (candidates-request):', err);
            }
        }
    },
};
