...
export default {
    async afterCreate(event) {
        ...
        strapi.log.info('Callback-request afterCreate triggered for ID:', result.id);
        ...
        `;
                ...
                await strapi.db
                    .query('api::callback-request.callback-request')
                    .update({
                        where: { id: result.id },
                        data: { emailSent: true },
                    });

                strapi.log.info(`Email отправлен (callback-request)`);
            } catch (err) {
                strapi.log.error('Ошибка отправки email (callback-request):', err);
            }
        }
    },
};
