...
export default {
    async afterCreate(event) {
        ...
        strapi.log.info('Compliance-request afterCreate triggered for ID:', result.id);
        ...
        `;
                ...
                await strapi.db
                    .query('api::compliance-request.compliance-request')
                    .update({
                        where: { id: result.id },
                        data: { emailSent: true },
                    });

                strapi.log.info(`Email отправлен (compliance-request)`);
            } catch (err) {
                strapi.log.error('Ошибка отправки email (compliance-request):', err);
            }
        }
    },
};
