...
export default {
    async afterCreate(event) {
        ...
        strapi.log.info('Contact-request afterCreate triggered for ID:', result.id);
        ...
        `;
                ...
                strapi.log.info(`Email отправлен (contact-request)`);
            } catch (err) {
                strapi.log.error('Ошибка отправки email (contact-request):', err);
            }
        }
    },
};
