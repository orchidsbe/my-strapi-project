...
export default {
    async afterCreate(event) {
        ...
        strapi.log.info('Corporate-solutions-request afterCreate triggered for ID:', result.id);
        ...
        `;
                ...
                await strapi.db
                    .query('api::corporate-solutions-request.corporate-solutions-request')
                    .update({
                        where: { id: result.id },
                        data: { emailSent: true },
                    });

                strapi.log.info(`Email отправлен (corporate-solutions-request)`);
            } catch (err) {
                strapi.log.error('Ошибка отправки email (corporate-solutions-request):', err);
            }
        }
    },
};
