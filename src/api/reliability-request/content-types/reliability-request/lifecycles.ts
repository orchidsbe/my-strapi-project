...
export default {
    async afterCreate(event) {
        ...
        strapi.log.info('Reliability Request afterCreate triggered for ID:', result.id);
        ...
        `;
                ...
                await strapi.db
                    .query('api::reliability-request.reliability-request')
                    .update({
                        where: { id: result.id },
                        data: { emailSent: true },
                    });

                strapi.log.info(`Email отправлен (Reliability Request)`);
            } catch (err) {
                strapi.log.error('Ошибка отправки email (Reliability Request):', err);
            }
        }
    },
};
