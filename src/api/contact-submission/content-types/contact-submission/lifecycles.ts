...
export default {
    async afterCreate(event) {
        ...
        strapi.log.info('Contact-submission afterCreate triggered for ID:', result.id);
        ...
        `;
                ...
                strapi.log.info(`Email отправлен (contact-submission)`);
            } catch (err) {
                strapi.log.error('Ошибка отправки email (contact-submission):', err);
            }
        }
    },
};
