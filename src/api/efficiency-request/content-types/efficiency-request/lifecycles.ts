...
export default {
    async afterCreate(event) {
        ...
        strapi.log.info('Efficiency-request afterCreate triggered for ID:', result.id);
        ...
        `;
                ...
                await strapi.db
                    .query('api::efficiency-request.efficiency-request')
                    .update({
                        where: { id: result.id },
                        data: { emailSent: true },
                    });

                strapi.log.info(`Email отправлен (efficiency-request)`);
            } catch (err) {
                strapi.log.error('Ошибка отправки email (efficiency-request):', err);
            }
        }
    },
};
