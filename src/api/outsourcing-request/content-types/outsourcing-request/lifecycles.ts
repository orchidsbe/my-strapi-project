...
export default {
    async afterCreate(event) {
        ...
        strapi.log.info('Outsourcing Request afterCreate triggered for ID:', result.id);
        ...
        if (!result.emailSent) {
          ...
          <table style="border-collapse: collapse; width: 100%;">
            ...
            ${result.processes ? `<tr><td style="border:1px solid #ccc;padding:4px;"><strong>Процессы</strong></td><td style="border:1px solid #ccc;padding:4px;">${result.processes}</td></tr>` : ''}
        ...
        `;
                ...
                await strapi.db
                    .query('api::outsourcing-request.outsourcing-request')
                    .update({
                        where: { id: result.id },
                        data: { emailSent: true },
                    });

                strapi.log.info(`Email отправлен (Outsourcing Request)`);
            } catch (err) {
                strapi.log.error('Ошибка отправки email (Outsourcing Request):', err);
            }
        }
    },
};
