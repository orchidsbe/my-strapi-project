...
export default {
    async afterCreate(event) {
        ...
        strapi.log.info('Learning Program afterCreate triggered for ID:', result.id);
        ...
        `;
                ...
                await strapi.db
                    .query('api::learning-program.learning-program')
                    .update({
                        where: { id: result.id },
                        data: { emailSent: true },
                    });

                strapi.log.info(`Email отправлен (Learning Program)`);
            } catch (err) {
                strapi.log.error('Ошибка отправки email (Learning Program):', err);
            }
        }
    },
};
