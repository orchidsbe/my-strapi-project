...
export default {
    async afterCreate(event) {
        ...
        strapi.log.info('Subscription afterCreate triggered for ID:', result.id);
        ...
        if (!result.emailSent) {
            try {
                ...
                const mailHtml = `
          <h2>Новая подписка: ${result.formName}</h2>
        ...
        `;
                ...
                await transporter.sendMail({
                    ...
                    subject: `Новая подписка: ${result.formName}`,
                    html: mailHtml,
                });
                ...
                strapi.log.info(`Email отправлен (Subscription)`);
            } catch (err) {
                strapi.log.error('Ошибка отправки email (Subscription):', err);
            }
        }
    },
};
