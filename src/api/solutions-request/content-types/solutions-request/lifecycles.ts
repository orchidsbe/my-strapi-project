...
export default {
    async afterCreate(event) {
        ...
        strapi.log.info('SolutionsRequest afterCreate triggered for ID:', result.id);
        ...
        `;
                ...
                await strapi.db
                    .query('api::solutions-request.solutions-request')
                    .update({
                        where: { id: result.id },
                        data: { emailSent: true },
                    });

                strapi.log.info(`Email отправлен (SolutionsRequest)`);
            } catch (err) {
                strapi.log.error('Ошибка отправки email (SolutionsRequest):', err);
            }
        }
    },
};
