...
export default {
    async afterCreate(event) {
        ...
        strapi.log.info('HR Consulting afterCreate triggered for ID:', result.id);
        ...
        if (!result.emailSent) {
          ...
          <table style="border-collapse: collapse; width: 100%;">
            ...
            ${result.userRequest ? `<tr><td style="border:1px solid #ccc;padding:4px;"><strong>Запрос</strong></td><td style="border:1px solid #ccc;padding:4px;">${result.userRequest}</td></tr>` : ''}
          </table>
        `;
                ...
                await strapi.db
                    .query('api::hr-consulting-request.hr-consulting-request')
                    .update({
                        where: { id: result.id },
                        data: { emailSent: true },
                    });

                strapi.log.info(`Email отправлен (HR Consulting)`);
            } catch (err) {
                strapi.log.error('Ошибка отправки email (HR Consulting):', err);
            }
        }
    },
};
