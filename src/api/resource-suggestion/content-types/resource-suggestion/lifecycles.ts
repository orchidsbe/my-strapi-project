...
export default {
    async afterCreate(event) {
        ...
        strapi.log.info('Resource Suggestion afterCreate triggered for ID:', result.id);
        ...
        `;
                ...
                await strapi.db
                    .query('api::resource-suggestion.resource-suggestion')
                    .update({
                        where: { id: result.id },
                        data: { emailSent: true },
                    });

                strapi.log.info(`Email отправлен (Resource Suggestion)`);
            } catch (err) {
                strapi.log.error('Ошибка отправки email (Resource Suggestion):', err);
            }
        }
    },
};
